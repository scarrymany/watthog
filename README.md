...
[![Currencies](https://img.shields.io/badge/₴%20₽%20$%20₸-4%20валюты-161b22?style=for-the-badge&labelColor=161b22&color=43d675)](#тариф)
...
[Установка](#установка) · [Два интерфейса](#два-интерфейса) · [Как пользоваться](#как-пользоваться) · [Как это работает](#как-это-работает) · [Точность](#точность) · [Тариф](#тариф) · [Поддержать](#поддержать-проект) · [English](README.en.md)
...
| 💵 **4 валюты из коробки** | Гривна, рубль, доллар и тенге с готовыми тарифами Украины, России, США и Казахстана - переключаются одним выбором |
...
watthog tariffs                      # справочник тарифов
watthog run --preset ru-msk-gas      # считать по московскому тарифу
```
...
| `--preset КЛЮЧ` | Тариф из справочника: `ua`, `ua-night`, `ru-msk-gas`, `ru-msk-electric`, `kz-almaty`, `kz-almaty-2`, `us` |
...
Поддерживаются **4 валюты**: гривна `₴`, рубль `₽`, доллар `$` и тенге `₸`. Для каждой
в программе уже заложен рабочий тариф, так что расход в деньгах считается сразу после
установки.

По умолчанию берётся украинский тариф - **4.32 ₴ за кВт·ч**, единая по всей стране цена
для населения. Переключить валюту можно одним выбором: в окне это выпадающий список
«Тариф из справочника», в консоли - пункт меню **5** или команда `watthog tariffs`.

| Ключ | Тариф | Цена | Источник |
|---|---|---|---|
| `ua` | 🇺🇦 Украина, население | **4.32 ₴** | постановление КМУ №632 от 31.05.2024 |
| `ua-night` | 🇺🇦 Украина, ночь 23:00-07:00 | **2.16 ₴** | половина дневного при двухзонном учёте |
| `ru-msk-gas` | 🇷🇺 Москва, газовая плита | **8.00 ₽**, с 01.10.2026 - 8.90 ₽ | Департамент экономической политики Москвы |
| `ru-msk-electric` | 🇷🇺 Москва, электроплита | **7.28 ₽**, с 01.10.2026 - 8.46 ₽ | Департамент экономической политики Москвы |
| `kz-almaty` | 🇰🇿 Алматы, 1 уровень | **29.34 ₸** | АО «Алатау Жарық Компаниясы», дифференцированный тариф |
| `kz-almaty-2` | 🇰🇿 Алматы, 2 уровень | **38.78 ₸** | сверх нормы потребления |
| `us` | 🇺🇸 США, в среднем | **0.1791 $** | EIA Electric Power Monthly, таблица 5.6.B |

Справочник знает даты вступления цен в силу, поэтому после 1 октября 2026 программа
сама начнёт считать по новому московскому тарифу - обновлять её для этого не нужно.

Тариф зависит от региона, типа счётчика, объёма потребления и категории жилья, поэтому
справочник - это отправная точка, а не истина. Любое значение заменяется своим в
настройках, и валюта тоже: там обычное текстовое поле, а не список из четырёх вариантов.
Вписать евро, злотый или любой другой знак ничто не мешает - справочник просто избавляет
от ручного ввода в самых частых случаях.

<div align="center">
<img src="docs/gui-settings.png" width="520" alt="Настройки и выбор тарифа">
</div>
...
| `tariff_per_kwh` | 4.32 | Цена киловатт-часа |
| `currency` | ₴ | Обозначение валюты: ₴, ₽, $, ₸ или любое своё |
